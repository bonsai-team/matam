    ...
    # Try to find a more recent LCA using at least min_proportion of the population
        ...
        for group_by_taxo_list in factor_taxo_list:
            for taxo in group_by_taxo_list:
                taxo_tab = taxo.split(';')
                #~ for i in xrange(1, len(taxo_tab)): # Here, there was a bug, but in fact we learned that we can have better results by setting the LCA level we want in advance
                for i in xrange(len(taxo_tab)): 
